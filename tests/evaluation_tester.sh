...
TEMP_DIR="/tmp/minishell_evaluation_$$"
...
ls -l | grep Makefile
echo hello | cat > $TEMP_DIR/pipe_out.txt && cat $TEMP_DIR/pipe_out.txt
"
...
echo This is a very long command with many arguments to test the parsing capabilities of the shell
"
...
run_test() {
    ...
    # Format the command for display (truncate if too long)
    ...
    printf "%-15s %-50s %-10s %-10s\n" "[$category]" "$display_cmd" "${GREEN}bash OK${RESET}" "${result/OK/${GREEN}OK${RESET}}${result/KO/${RED}KO${RESET}}"
    
    # Save detailed results to file
    echo "[$category] $command: $result" >> "$RESULTS_FILE"
    if [ "$result" = "KO" ]; then
...
main() {
    ...
    echo "MINISHELL EVALUATION RESULTS" > "$RESULTS_FILE"
    echo "==================================" >> "$RESULTS_FILE"
    echo "Date: $(date)" >> "$RESULTS_FILE"
    echo "" >> "$RESULTS_FILE"
    
    # Print header
    echo -e "${BLUE}${BOLD}MINISHELL EVALUATION TEST RESULTS${RESET}"
    echo -e "${YELLOW}----------------------------------------${RESET}"
    printf "%-15s %-50s %-10s %-10s\n" "CATEGORY" "COMMAND" "BASH" "MINISHELL"
    echo -e "${YELLOW}----------------------------------------${RESET}"
    ...
    for category in "BASIC_COMMANDS" "BUILTINS" "ENVIRONMENT" "QUOTES" "REDIRECTIONS" "PIPES" "ERRORS" "COMPLEX"; do
